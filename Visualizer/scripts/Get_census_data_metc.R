...
print(paste("Writing summaries to",settings$visualizer_summaries))
...
acs_veh$county = counties$CO_NAME[match(substr(acs_veh$GEOID, 0, 5),counties$county_fips)]
...
auto_ownership = rbind(auto_ownership, 
                       cbind(county = "Total", dcast(acs_veh[variable %in% c('B08201_002', 'B08201_003', 'B08201_004', 'B08201_005', 'B08201_006')],
...
acs_mode = acs_mode[substr(GEOID, 0, 5) %in% counties[, county_fips]]
acs_mode[, GEOID := as.numeric(GEOID)]
acs_mode$county = counties$CO_NAME[match(substr(acs_mode$GEOID, 0, 5),counties$county_fips)]

wfh = dcast(acs_mode[variable %in% c("B08006_001", "B08006_017")], county ~ variable, value.var = 'estimate', fun = sum )

colnames(wfh) = c("District","Workers","WFH")

totalwfh = data.frame("Total", sum(wfh$Workers) ,sum(wfh$WFH))

totalwfh

colnames(totalwfh) = colnames(wfh)
wfh_summary = rbind(wfh, totalwfh)

fwrite(wfh_summary, file.path(settings$visualizer_summaries, 'wfh_summary.csv'))
