...
modeCodes <- data.frame(code = c(1, 2, 3, 4, 5, 6, 6, 6, 7, 7, 7, 8, 8, 8, 9,9, 9, 10),
                        name = c("DRIVEALONE", "SHARED2", "SHARED3", "WALK", "BIKE",
                                 ...
                                 "TAXI", "TNC_SINGLE", "TNC_SHARED", #9 = ridehail
                                 "SCHOOLBUS"
                                 )) 
...
pertype_xwalk = data.frame(ptype_orig = c(1:8), 
                           ptype_update = c(8, 7, 6, 3, 1, 2, 4, 5))

per$ptype_orig = per$ptype

per = merge(per, pertype_xwalk, by.x = 'ptype_orig', by.y  = 'ptype_orig')

per$ptype = per$ptype_update
...
write.csv(pertypeDistbn, file.path(WD, "pertypeDistbn.csv"), row.names = TRUE)
...
write.csv(tlfd_work_df, file.path(WD, "workTLFD.csv"), row.names = F)
write.csv(tlfd_univ_df, file.path(WD, "univTLFD.csv"), row.names = F)
write.csv(tlfd_schl_df, file.path(WD, "schlTLFD.csv"), row.names = F)
...
write.csv(countyFlows, file.path(WD, "countyFlows.csv"), row.names = T)
...
all_tours$TOUR_DUR_BIN_RECODE = all_tours$ANCHOR_ARRIVE_BIN_RECODE - all_tours$ANCHOR_DEPART_BIN_RECODE + 1
...
write.csv(stopFreqModel_summary, file.path(WD, "stopFreqModel_summary.csv"), row.names = T)
...
# write.table("Non-Mandatory Tours for Active Persons with 0 Mandatory Tour", file.path(WD, "indivNMTourFreq.csv"), sep = ",", row.names = F, append = T)
# write.table(freq_nmtours_nomand, file.path(WD, "indivNMTourFreq.csv"), sep = ",", row.names = F, append = TRUE)
...
write.csv(toursPertypeDistbn, file.path(WD, "toursPertypeDistbn.csv"), row.names = TRUE)
...
write.csv(totaltoursPertypeDistbn, file.path(WD, "total_tours_by_pertype_vis.csv"), row.names = F)
...
write.csv(tours_pertype_purpose, file.path(WD, "tours_pertype_purpose.csv"), row.names = TRUE)
...
write.csv(nm_tour_rates, file.path(WD, "nm_tour_rates.csv"), row.names = F)
...
write.csv(tours_purpose_type, file.path(WD, "tours_purpose_type.csv"), row.names = TRUE)

# DAP by pertype
dapSummary <- count(per, c("PERTYPE", "cdap_activity"), "finalweight")
write.csv(dapSummary, file.path(WD, "dapSummary.csv"), row.names = TRUE)
...
write.csv(dapSummary_vis, file.path(WD, "dapSummary_vis.csv"), row.names = TRUE)

# HHSize X Joint
hhsizeJoint <- count(hh[hh$HHSIZE>=2,], c("HHSIZE", "JOINT"), "finalweight")
write.csv(hhsizeJoint, file.path(WD, "hhsizeJoint.csv"), row.names = TRUE)
...
write.csv(mtfSummary, file.path(WD, "mtfSummary.csv"))
#write.csv(tours, file.path(WD, "tours_test.csv"))
...
write.table(inmSummary, file.path(WD, "innmSummary.csv"), col.names=TRUE, sep=",")
...
write.csv(inmSummary_vis, file.path(WD, "inmSummary_vis.csv"))
...
# write.table(jtfSummary, file.path(WD, "jtfSummary.csv"), col.names=TRUE, sep=",")
# write.table(jointComp, file.path(WD, "jtfSummary.csv"), col.names=TRUE, sep=",", append=TRUE)
# write.table(jointPartySize, file.path(WD, "jtfSummary.csv"), col.names=TRUE, sep=",", append=TRUE)
# write.table(jointCompPartySize, file.path(WD, "jtfSummary.csv"), col.names=TRUE, sep=",", append=TRUE)
# write.table(jointToursHHSize, file.path(WD, "jtfSummary.csv"), col.names=TRUE, sep=",", append=TRUE)
...
write.csv(jtf, file.path(WD, "jtf.csv"), row.names = F)
write.csv(jointComp, file.path(WD, "jointComp.csv"), row.names = F)
write.csv(jointPartySize, file.path(WD, "jointPartySize.csv"), row.names = F)
write.csv(jointCompPartySizeProp, file.path(WD, "jointCompPartySize.csv"), row.names = F)
write.csv(jointToursHHSizeProp, file.path(WD, "jointToursHHSize.csv"), row.names = F)
...
tod1 <- wtd.hist(tours$start_hour[tours$TOURPURP==1], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==1])
tod2 <- wtd.hist(tours$start_hour[tours$TOURPURP==2], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==2])
tod3 <- wtd.hist(tours$start_hour[tours$TOURPURP==3], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==3])
tod4 <- wtd.hist(tours$start_hour[tours$TOURPURP==4], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==4])
todi56 <- wtd.hist(tours$start_hour[tours$TOURPURP>=5 & tours$TOURPURP<=6], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=5 & tours$TOURPURP<=6])
todi789 <- wtd.hist(tours$start_hour[tours$TOURPURP>=7 & tours$TOURPURP<=9], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=7 & tours$TOURPURP<=9])
todj56 <- wtd.hist(unique_joint_tours$start_hour[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6])
todj789 <- wtd.hist(unique_joint_tours$start_hour[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9])
tod15 <- wtd.hist(tours$start_hour[tours$TOURPURP==10], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==10])
...
colnames(todDepProfile) <- c("work", "univ", "sch", "esc", "imain", "idisc", 
                             "jmain", "jdisc", "atwork")
write.csv(todDepProfile, file.path(WD, "todDepProfile.csv"))
...
tod1 <- wtd.hist(tours$end_hour[tours$TOURPURP==1], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==1])
tod2 <- wtd.hist(tours$end_hour[tours$TOURPURP==2], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==2])
tod3 <- wtd.hist(tours$end_hour[tours$TOURPURP==3], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==3])
tod4 <- wtd.hist(tours$end_hour[tours$TOURPURP==4], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==4])
todi56 <- wtd.hist(tours$end_hour[tours$TOURPURP>=5 & tours$TOURPURP<=6], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=5 & tours$TOURPURP<=6])
todi789 <- wtd.hist(tours$end_hour[tours$TOURPURP>=7 & tours$TOURPURP<=9], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=7 & tours$TOURPURP<=9])
todj56 <- wtd.hist(unique_joint_tours$end_hour[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6])
todj789 <- wtd.hist(unique_joint_tours$end_hour[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9])
tod15 <- wtd.hist(tours$end_hour[tours$TOURPURP==10], breaks = seq(5,23, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==10])
...
colnames(todArrProfile) <- c("work", "univ", "sch", "esc", "imain", "idisc", 
                             "jmain", "jdisc", "atwork")
write.csv(todArrProfile, file.path(WD, "todArrProfile.csv"))
...
tod1 <- wtd.hist(tours$tourdur[tours$TOURPURP==1], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==1])
tod2 <- wtd.hist(tours$tourdur[tours$TOURPURP==2], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==2])
tod3 <- wtd.hist(tours$tourdur[tours$TOURPURP==3], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==3])
tod4 <- wtd.hist(tours$tourdur[tours$TOURPURP==4], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==4])
todi56 <- wtd.hist(tours$tourdur[tours$TOURPURP>=5 & tours$TOURPURP<=6], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=5 & tours$TOURPURP<=6])
todi789 <- wtd.hist(tours$tourdur[tours$TOURPURP>=7 & tours$TOURPURP<=9], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP>=7 & tours$TOURPURP<=9])
todj56 <- wtd.hist(unique_joint_tours$tourdur[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=5 & unique_joint_tours$JOINT_PURP<=6])
todj789 <- wtd.hist(unique_joint_tours$tourdur[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = unique_joint_tours$finalweight[unique_joint_tours$JOINT_PURP>=7 & unique_joint_tours$JOINT_PURP<=9])
tod15 <- wtd.hist(tours$tourdur[tours$TOURPURP==10], breaks = seq(1,19, by=1), freq = NULL, right=FALSE, weight = tours$finalweight[tours$TOURPURP==10])
...
colnames(todDurProfile) <- c("work", "univ", "sch", "esc", "imain", "idisc", 
                             "jmain", "jdisc", "atwork")
write.csv(todDurProfile, file.path(WD, "todDurProfile.csv"))
...
write.csv(todProfile_vis, file.path(WD, "todProfile_vis.csv"), row.names = F)
...
write.csv(tmodeAS0Profile, file.path(WD, "tmodeAS0Profile.csv"))
...
write.csv(tmodeAS1Profile, file.path(WD, "tmodeAS1Profile.csv"))
...
write.csv(tmodeAS2Profile, file.path(WD, "tmodeAS2Profile.csv"))
...
write.csv(tourDistProfile, file.path(WD, "nonMandTourDistProfile.csv"))
...
write.csv(tourDistProfile_vis, file.path(WD, "tourDistProfile_vis.csv"), row.names = F)
...
write.csv(nonMandTourLengths, file.path(WD, "nonMandTourLengths.csv"), row.names = F)
...
write.csv(stopFreq, file.path(WD, "stopFreqOutProfile.csv"))
...
stopFreq <- data.frame(stopfreq1$counts, stopfreq2$counts, stopfreq3$counts, stopfreq4$counts, stopfreqi56$counts
                       , stopfreqi789$counts, stopfreqj56$counts, stopfreqj789$counts, stopfreq10$counts)
colnames(stopFreq) <- c("work", "univ", "sch", "esco","imain", "idisc", "jmain", "jdisc", "atwork")
write.csv(stopFreq, file.path(WD, "stopFreqInbProfile.csv"))
...
write.csv(stopfreqDir_vis, file.path(WD, "stopfreqDir_vis.csv"), row.names = F)
...
stopFreq <- data.frame(stopfreq1$counts, stopfreq2$counts, stopfreq3$counts, stopfreq4$counts, stopfreqi56$counts
                       , stopfreqi789$counts, stopfreqj56$counts, stopfreqj789$counts, stopfreq10$counts)
colnames(stopFreq) <- c("work", "univ", "sch", "esco","imain", "idisc", "jmain", "jdisc", "atwork")
write.csv(stopFreq, file.path(WD, "stopFreqTotProfile.csv"))
...
write.csv(stopFreq_vis, file.path(WD, "stopfreq_total_vis.csv"), row.names = F)
...
stopFreq <- data.frame(stopfreq1$counts, stopfreq2$counts, stopfreq3$counts, stopfreq4$counts, stopfreqi56$counts
                       , stopfreqi789$counts, stopfreqj56$counts, stopfreqj789$counts, stopfreq10$counts)
colnames(stopFreq) <- c("work", "univ", "sch", "esco","imain", "idisc", "jmain", "jdisc", "atwork")
write.csv(stopFreq, file.path(WD, "stopPurposeByTourPurpose.csv"))
...
write.csv(stopFreq_vis, file.path(WD, "stoppurpose_tourpurpose_vis.csv"), row.names = F)
...
stopFreq <- data.frame(stopfreq1$counts, stopfreq2$counts, stopfreq3$counts, stopfreq4$counts, stopfreqi56$counts
                       , stopfreqi789$counts, stopfreqj56$counts, stopfreqj789$counts, stopfreq10$counts)
colnames(stopFreq) <- c("work", "univ", "sch", "esco","imain", "idisc", "jmain", "jdisc", "atwork")
write.csv(stopFreq, file.path(WD, "stopOutOfDirectionDC.csv"))
...
write.csv(stopDC_vis, file.path(WD, "stopDC_vis.csv"), row.names = F)
...
write.csv(avgStopOutofDirectionDist, file.path(WD, "avgStopOutofDirectionDist_vis.csv"), row.names = F)

#Stop Departure Time
stopfreq1 <- wtd.hist(stops$depart_hour[stops$TOURPURP==1], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = stops$finalweight[stops$TOURPURP==1])
stopfreq2 <- wtd.hist(stops$depart_hour[stops$TOURPURP==2], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = stops$finalweight[stops$TOURPURP==2])
stopfreq3 <- wtd.hist(stops$depart_hour[stops$TOURPURP==3], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = stops$finalweight[stops$TOURPURP==3])
stopfreq4 <- wtd.hist(stops$depart_hour[stops$TOURPURP==4], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = stops$finalweight[stops$TOURPURP==4])
stopfreqi56  <- wtd.hist(stops$depart_hour[stops$TOURPURP>=5 & stops$TOURPURP<=6], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = stops$finalweight[stops$TOURPURP>=5 & stops$TOURPURP<=6])
stopfreqi789 <- wtd.hist(stops$depart_hour[stops$TOURPURP>=7 & stops$TOURPURP<=9], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = stops$finalweight[stops$TOURPURP>=7 & stops$TOURPURP<=9])
stopfreqj56 <- wtd.hist(jstops$depart_hour[jstops$TOURPURP>=5 & jstops$TOURPURP<=6], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = jstops$finalweight[jstops$TOURPURP>=5 & jstops$TOURPURP<=6])
stopfreqj789 <- wtd.hist(jstops$depart_hour[jstops$TOURPURP>=7 & jstops$TOURPURP<=9], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = jstops$finalweight[jstops$TOURPURP>=7 & jstops$TOURPURP<=9])
stopfreq10 <- wtd.hist(stops$depart_hour[stops$TOURPURP==10], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = stops$finalweight[stops$TOURPURP==10])

stopFreq <- data.frame(stopfreq1$counts, stopfreq2$counts, stopfreq3$counts, stopfreq4$counts, stopfreqi56$counts
                       , stopfreqi789$counts, stopfreqj56$counts, stopfreqj789$counts, stopfreq10$counts)
colnames(stopFreq) <- c("work", "univ", "sch", "esco","imain", "idisc", "jmain", "jdisc", "atwork")
write.csv(stopFreq, file.path(WD, "stopDeparture.csv"))
...
#Trip Departure Time
stopfreq1 <- wtd.hist(trips$depart_hour[trips$TOURPURP==1], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TOURPURP==1])
stopfreq2 <- wtd.hist(trips$depart_hour[trips$TOURPURP==2], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TOURPURP==2])
stopfreq3 <- wtd.hist(trips$depart_hour[trips$TOURPURP==3], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TOURPURP==3])
stopfreq4 <- wtd.hist(trips$depart_hour[trips$TOURPURP==4], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TOURPURP==4])
stopfreqi56 <- wtd.hist(trips$depart_hour[trips$TOURPURP>=5 & trips$TOURPURP<=6], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TOURPURP>=5 & trips$TOURPURP<=6])
stopfreqi789 <- wtd.hist(trips$depart_hour[trips$TOURPURP>=7 & trips$TOURPURP<=9], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TOURPURP>=7 & trips$TOURPURP<=9])
stopfreqj56 <- wtd.hist(jtrips$depart_hour[jtrips$TOURPURP>=5 & jtrips$TOURPURP<=6], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = jtrips$finalweight[jtrips$TOURPURP>=5 & jtrips$TOURPURP<=6])
stopfreqj789 <- wtd.hist(jtrips$depart_hour[jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = jtrips$finalweight[jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9])
stopfreq10 <- wtd.hist(trips$depart_hour[trips$TOURPURP==10], breaks =  c(seq(5,23, by=1), 9999), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TOURPURP==10])

stopFreq <- data.frame(stopfreq1$counts, stopfreq2$counts, stopfreq3$counts, stopfreq4$counts, stopfreqi56$counts
                       , stopfreqi789$counts, stopfreqj56$counts, stopfreqj789$counts, stopfreq10$counts)
colnames(stopFreq) <- c("work", "univ", "sch", "esco","imain", "idisc", "jmain", "jdisc", "atwork")
write.csv(stopFreq, file.path(WD, "tripDeparture.csv"))
...
write.csv(stopTripDep_vis, file.path(WD, "stopTripDep_vis.csv"), row.names = F)
...
# tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==11])
# tripmode12 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==12], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==1 & trips$TOURMODE==12])
...
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_Work.csv"))
...
# tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==2 & trips$TOURMODE==11])
...
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", "tourmode6", 
                               "tourmode7", "tourmode8", "tourmode9", "tourmode10")
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_Univ.csv"))
...
# tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==3 & trips$TOURMODE==11])
...
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", "tourmode6",
                               "tourmode7", "tourmode8", "tourmode9", "tourmode10")
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_Schl.csv"))
...
# tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=4 & trips$TOURPURP<=6 & trips$TOURMODE==11])
...
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", 
                               "tourmode6", "tourmode7", "tourmode8", "tourmode9", "tourmode10")
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_iMain.csv"))
...
# tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP>=7 & trips$TOURPURP<=9 & trips$TOURMODE==11])
...
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", 
                               "tourmode6", "tourmode7", "tourmode8", "tourmode9", "tourmode10")
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_iDisc.csv"))
...
# tripmode11 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=4 & jtrips$TOURPURP<=6 & jtrips$TOURMODE==11])
...
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", 
                               "tourmode6", "tourmode7", "tourmode8", "tourmode9", "tourmode10")
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_jMain.csv"))
...
# tripmode11 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURPURP>=7 & jtrips$TOURPURP<=9 & jtrips$TOURMODE==11])
...
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", 
                               "tourmode6", "tourmode7", "tourmode8", "tourmode9", "tourmode10")
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_jDisc.csv"))
...
# tripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURPURP==10 & trips$TOURMODE==11])
...
colnames(tripModeProfile) <- c("tourmode1", "tourmode2", "tourmode3", "tourmode4", "tourmode5", 
                               "tourmode6", "tourmode7", "tourmode8", "tourmode9", "tourmode10")
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_AtWork.csv"))
...
# itripmode11 <- wtd.hist(trips$TRIPMODE[trips$TRIPMODE>0 & trips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = trips$finalweight[trips$TRIPMODE>0 & trips$TOURMODE==11])
...
# jtripmode11 <- wtd.hist(jtrips$TRIPMODE[jtrips$TRIPMODE>0 & jtrips$TOURMODE==11], breaks = seq(1,11, by=1), freq = NULL, right=FALSE, weight = jtrips$numpart_wgt[jtrips$TRIPMODE>0 & jtrips$TOURMODE==11])
...
write.csv(tripModeProfile, file.path(WD, "tripModeProfile_Total.csv"))
...
#write.csv(tripModeProfile_vis, file.path(WD, "tripModeProfile_vis.csv"), row.names = F)
...
write.csv(totals_df, file.path(WD, "totals.csv"), row.names = F)

# HH Size distribution
hhSizeDist <- count(hh[hh$TYPE == 1,], c("HHSIZE"), "finalweight") #only counting non-GQ households
write.csv(hhSizeDist, file.path(WD, "hhSizeDist.csv"), row.names = F)
...
write.table(trips_flow, file.path(WD, "trips_flow.csv"), sep = ",", row.names = F)
...
trips_transit <- all_trips[all_trips$TRIPMODE %in% c(6,7),]
trips_transit$access_mode[trips_transit$TRIPMODE==6] <- "walk"
trips_transit$access_mode[trips_transit$TRIPMODE==7] <- "drive"
...
write.table(trips_transit_summary, file.path(WD,"trips_transit_summary.csv"), sep = ",", row.names = F)
...
write.csv(tlfd_transit_df, file.path(WD, "transitTLFD.csv"), row.names = F)
...
tlfd_transit_df[is.na(tlfd_transit_df)] <- 0
write.csv(tlfd_transit_df, file.path(WD, "transitTLFD10.csv"), row.names = F)
...
# write.table("work_Tours", file.path(WD, "work_Tours.csv"), sep = ",")
# write.table(work_Tours, file.path(WD, "work_Tours.csv"), sep = ",", row.names = F, append = T)
...
# write.table("workersParkZone", file.path(WD, "workersParkZone.csv"), sep = ",")
# write.table(workersParkZone, file.path(WD, "workersParkZone.csv"), sep = ",", row.names = F, append = T)
