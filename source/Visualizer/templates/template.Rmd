...
sample_rate <- ifelse(IS_BASE_SURVEY=="Yes", "100", as.character(BASE_SAMPLE_RATE*100))
...
census_df = census_data[[which(census_csv_names=="hhSizeCensus")]]

build_scen_name = BUILD_SCENARIO_NAME
if(IS_BASE_SURVEY=="Yes"){
  build_scen_name = CENSUS_SCENARIO_NAME
}
...
p <- plotly_bar_plotter(data = std_DF, xlabel = "HH Size", ylabel = "Percent", ynames = c(BASE_SCENARIO_NAME, build_scen_name), percent = T)
p
...
if(IS_BASE_SURVEY=="Yes"){
  base_df <- census_data[[which(census_csv_names=="autoOwnershipcensus_2021")]]
}else{
...
# sd.autoown <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "HHVEH", y = c("freq"))
...
p <- plotly_bar_plotter(data = sd.autoown, xlabel = "Number of Vehicles", ylabel = "Percent", ynames = c(BASE_SCENARIO_NAME, build_scen_name), percent = T, height = 225)
...
```{r Chart_WFH}
base_df <- census_data[[which(census_csv_names == "census_acs_2021_wfh")]]
colnames(base_df) = c("District","tot_wrk","wfh","share")
...
std_DF <- merge(base_df[1:19,c("District", "share")], build_df[1:19,c("District", "share")], by = "District")

colnames(std_DF) <- c("xvar", "prop1", "prop2")

p <- plotly_bar_plotter(data = std_DF, xlabel = "District", ylabel = "Percent WFH", ynames = c(BASE_SCENARIO_NAME, build_scen_name), percent = T, height = 275, tickangle = -320, bottom_offset = 25)
p
...
df_list = list(build_df1, build_df2, build_df3)
build_df = Reduce(function(x, y) merge(x, y, all=TRUE, by = c('distbin', 'variable')), df_list)
build_df[is.na(build_df)] = 0
...
build_scen_name = BUILD_SCENARIO_NAME
if(IS_BASE_SURVEY=="Yes"){
  base_df <- census_data[[which(census_csv_names=="countyFlowsCensus2021")]]
  build_scen_name = CENSUS_SCENARIO_NAME
  rownames(base_df) = base_df$O_County_Name
  base_df = base_df[1:(nrow(base_df) - 1), 2:(ncol(base_df) - 1)]
  nworkers = formatC(sum(base_df), format="f", big.mark=",", digits=0)
  diag = chorddiag(as.matrix(base_df), palette = 'Spectral')
  }else{
...
cat("County - County Flow of Workers -", build_scen_name, ':', nworkers, 'workers in region')
...
```{r job_wrk1a}
# bscols(widths=c(6,6),
#   include_graphics(file.path(SYSTEM_JPEG_PATH, "workersByCounty.jpeg")),
#   include_graphics(file.path(SYSTEM_JPEG_PATH, "workersByTAZ.jpeg"))
# )

# knitr::include_graphics(file.path(SYSTEM_JPEG_PATH, "workersByCounty.jpeg"))
...
x_text = 0.55 * max(df$freq)
y1_text = 0.1 * max(df$freq_build)
y2_text = 0.05 * max(df$freq_build)

ggplot(df, aes(x = freq, y = freq_build, label = WDISTRICT)) + geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  geom_text(vjust = 0, nudge_y = 0.1, check_overlap = TRUE) +
  scale_x_log10("SEDATA Input", labels = comma, limits = c(10000, max(df$freq, df$freq_build))) +
  scale_y_log10("ActivitySim", labels = comma, limits = c(10000, max(df$freq, df$freq_build))) +
  annotate("text", x=x_text, y=c(y1_text, y2_text), label= c(text1, text2))
```
...
ggplot(df, aes(x = base, y = build)) + geom_point(alpha = 0.3) +
  ...
  annotate("text", x=15000, y=c(4000, 1500), label= c(text1, text2))
```
...
labels <- sprintf(
  "<strong>%s</strong><br/><strong>%s %.2f %s</strong><br/> %s<br/>Census 0A: %s%s, Model 0A: %s%s", ct_zero_auto_shp@data$TRACTCE, ct_zero_auto_shp@data$txtCmm2, ct_zero_auto_shp@data$Dff_ZrA, "%", ct_zero_auto_shp@data$txtCmm1, round(ct_zero_auto_shp@data$Cns_A0P, 2), "%", round(ct_zero_auto_shp@data$Mdl_A0P, 2), "%") %>% lapply(htmltools::HTML)
...
p <- plotly_bar_plotter(data = std_DF, xlabel = "HH Size", ylabel = "Percent of Households", ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME), percent = F, tickformat = ".1%", hoverformat = ".1%")
p
...
base_df <- base_df[-1,]
build_df <- build_df[-1,colnames(base_df)]
colnames(build_df) <- colnames(base_df)
...
p1 <- plotly_density_plotter(sd.purpose, index = "one", xlabel = "Miles", percent = T,
                             tickvals = seq(1,40), ticktext = c(seq(1,40), "40pl"))
...
Each tour is assigned a time period of departure (time leaving home or work) and arrival (time arriving back at home or work). The entire day is divided into 48 half-hour bins.
...
base_df$purpose <- as.character(base_df$purpose)
base_df$purpose <- purpose_type_df$name[match(base_df$purpose, purpose_type_df$code)]
# if(BASE_SCENARIO_NAME == "SEMCOG_HTS") {
  #build_df <- build_data[[which(build_csv_names=="tmodeProfile_vis_HTS")]]
# } else {
...
The entire day is divided into 48 half-hour bins.
...
# build_df <- build_data[[which(build_csv_names=="tripModeProfile_vis_HTS")]]

colnames(build_df) <- colnames(base_df)

# change purpose names to standard format
# base_df$tripmode <- as.integer(base_df$tripmode)
# build_df$tripmode <- as.integer(build_df$tripmode)
# base_df$purpose <- as.character(base_df$purpose)
# build_df$purpose <- as.character(build_df$purpose)
base_df$purpose <- purpose_type_df$name[match(base_df$purpose, purpose_type_df$code)]
build_df$purpose <- purpose_type_df$name[match(build_df$purpose, purpose_type_df$code)]
...
Count vs Volume: All Day{data-navmenu="Hwy Assignment"}
============================================
...
```{r vmtcomp_dy}
  df = rbind.data.frame(cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$asnvmt_DAILY, "source" = "Model"), cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$obsvmt_DAILY, "source" = "Observed"))
df$VMT = as.numeric(df$VMT)

base_df = cbind.data.frame("FTYPE" = vmtcomp$FTYPE, "value" = vmtcomp$obsvmt_DAILY, "source" = "Observed")
base_df$grp_var = base_df$FTYPE
build_df = cbind.data.frame("FTYPE" = vmtcomp$FTYPE, "value" = vmtcomp$asnvmt_DAILY, "source" = "Model")
build_df$grp_var = build_df$FTYPE
  ...
  #   scale_y_continuous("Vehicle Miles of Travel", labels = comma) 
    ...
    sd.dy_vmt <- get_standardDF(data_df1=base_df, data_df2=build_df, x="FTYPE", y = c("value"), grp = "grp_var", shared = T)

p <- plotly_bar_plotter(data = sd.dy_vmt, height = 700, xlabel = "Facility Type", ylabel = "VMT", ynames = c("Observed", BUILD_SCENARIO_NAME), percent = F, tickvals = seq(1,length(vmtcomp$FTYPE)), ticktext = vmtcomp$FTYPE, bottom_offset = 75)
...
Count vs Volume: AM{data-navmenu="Hwy Assignment"}
============================================
...
ggplot(hnet[hnet$AM_CNT > 0,], aes(x = AM_CNT, y = AM_ASN)) + geom_point() +
	geom_abline(slope = 1, intercept = 0, color = "red") +
	scale_x_continuous("AM Count", labels = comma) +
	scale_y_continuous("AM Assignment", labels = comma)
```
...
### Assigned VMT Statistics{data-height=575}
```{r vmtcomp_am}
  df = rbind.data.frame(cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$asnvmt_AM, "source" = "Model"), cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$obsvmt_AM, "source" = "Observed"))
df$VMT = as.numeric(df$VMT)
...
Count vs Volume: MD{data-navmenu="Hwy Assignment"}
============================================
...
### Assigned VMT Statistics{data-height=575}
```{r vmtcomp_md}
  df = rbind.data.frame(cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$asnvmt_MD, "source" = "Model"), cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$obsvmt_MD, "source" = "Observed"))
df$VMT = as.numeric(df$VMT)
...
Count vs Volume: PM{data-navmenu="Hwy Assignment"}
============================================
...
ggplot(hnet[hnet$PM_CNT > 0,], aes(x = PM_CNT, y = PM_ASN)) + geom_point() +
	geom_abline(slope = 1, intercept = 0, color = "red") +
	scale_x_continuous("PM Count", labels = comma) +
	scale_y_continuous("PM Assignment", labels = comma)
```
...
### Assigned VMT Statistics{data-height=575}
```{r vmtcomp_pm}
  df = rbind.data.frame(cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$asnvmt_PM, "source" = "Model"), cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$obsvmt_PM, "source" = "Observed"))
df$VMT = as.numeric(df$VMT)
...
Count vs Volume: NT{data-navmenu="Hwy Assignment"}
============================================
...
ggplot(hnet[hnet$NT_CNT > 0,], aes(x = NT_CNT, y = NT_ASN)) + geom_point() +
	geom_abline(slope = 1, intercept = 0, color = "red") +
	scale_x_continuous("Nighttime Count", labels = comma) +
	scale_y_continuous("Nighttime Assignment", labels = comma)
```
...
### Assigned VMT Statistics{data-height=575}
```{r vmtcomp_nt}
  df = rbind.data.frame(cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$asnvmt_NT, "source" = "Model"), cbind("FTYPE" = vmtcomp$FTYPE, "VMT" = vmtcomp$obsvmt_NT, "source" = "Observed"))
df$VMT = as.numeric(df$VMT)
...
Total Assignment Summaries{data-navmenu="Hwy Assignment"}
============================================
...
### Vehicle Miles of Travel{data-height=5750}
```{r asn_vmt}

build_df <- vmtsum[vmtsum$Period == "DAILY" & vmtsum$COUNTY != 10,]

build_df$CNAME = assign_county[build_df$COUNTY]

base_df = build_df
base_df$vmt = 0

#FTYPE,COUNTY,Period,nLinks,vmt

sd.asnvmt <- get_standardDF(data_df1 = base_df, data_df2 = build_df, x = "FTYPE", y = c("vmt"), grp = c("CNAME"), shared = TRUE)

p <- plotly_bar_plotter(data = sd.asnvmt, xlabel = "Facility Type", ylabel = "VMT", percent = T, height = 225, ynames = c(BASE_SCENARIO_NAME, BUILD_SCENARIO_NAME))

bscols(widths=c(3,9),
  list(
    filter_select("asnvmt_cname", "Select County", sd.asnvmt, ~grp_var, multiple=F)),
    p
)

```

Transit Boardings: All Day{data-navmenu="Trn Assignment"}
============================================
