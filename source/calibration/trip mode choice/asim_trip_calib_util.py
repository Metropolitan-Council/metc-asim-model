...
# output_calibration_transit_modes = [
    ...
    # 'RIDEHAIL',
# ]
output_calibration_transit_modes = ['TRNWALKACCESS',
    'TRNDRIVEACCESS',]
...
output_calibration_mode_to_coef_name_dict_tour = {
    'DRIVEALONE': ['sov'],
    'SHARED2': ['sr2'],
    'SHARED3': ['sr3p'],
    'WALK': ['walk'],
    ...
    'TRNDRIVEACCESS': ['pnr_transit', 'drive_transit'],
    'SCHOOL_BUS': ['schoolbus'],
    'RIDEHAIL': ['taxi', 'tnc_single', 'tnc_shared', 'ridehail'],
    total_keyword: ['all'],
}

output_calibration_mode_to_coef_name_dict_trip = {
    'DRIVEALONE': ['sov'],
    'SHARED2': ['sr2'],
    'SHARED3': ['sr3', 'sr3p'],
    'WALK':['walk'],
...
output_calibration_purposes_to_coef_names_dict = {
    ...
    'school': ['school'],
    'atwork': ['atwork'],
    'ind_maint': ['shopping', 'escort', 'othmaint', 'maintenance'],
    'ind_discr': ['social', 'eatout', 'othdiscr', 'discretionary'],
    'joint_maint': ['shopping', 'escort', 'othmaint', 'maintenance'],
    'joint_discr': ['social', 'eatout', 'othdiscr', 'discretionary'],
...
# survey_table_purpose_col = 'TOURPURP'
# survey_table_trip_mode_col = 'TRIPMODE'
# added for trip mc calibration
# survey_table_tour_mode_col = 'TOURMODE'
...
calib_trip_mode_to_vis_dict = {  # tour mode choice vis
...
calib_tour_mode_to_vis_dict = {  # trip mode choice vis
    'DRIVEALONE': 'Auto SOV',
...
def check_input_dictionaries_for_consistency():
    ...
    for purpose in list(asim_to_calib_purpose_dict.values()):
        assert purpose in output_calibration_trip_purposes, f"ActivitySim purpose {purpose} not in calibration"
    for purpose in list(survey_to_calib_purposes_dict.values()):
        assert purpose in output_calibration_trip_purposes, f"Survey purpose {purpose} not in calibration"
...
def map_asim_trip_table_to_calib(trips_df):
    ...
    trips_df.loc[(trips_df['calib_tour_purpose'] == 'ind_maint')
                 & (trips_df['tour_category'] == 'joint'), 'calib_tour_purpose'] = 'joint_maint'
    trips_df.loc[(trips_df['calib_tour_purpose'] == 'ind_discr')
                 & (trips_df['tour_category'] == 'joint'), 'calib_tour_purpose'] = 'joint_discr'
...
def read_trip_mode_choice_calibration_file(trip_mode_choice_calib_targets_file):
    ...
    # Added ASR because there is "Other" in the targets that shouldn't be there
    trip_mc_calib_df = trip_mc_calib_df[trip_mc_calib_df[survey_table_trip_mode_col].isin(survey_to_calib_trip_mode_dict.keys())]
    trip_mc_calib_df = trip_mc_calib_df[trip_mc_calib_df[survey_table_tour_mode_col].isin(survey_to_calib_tour_mode_dict.keys())]
    # # #
...
def write_visualizer_calibration_target_table(full_trip_mc_calib_target_tables, asim_trips_df, output_dir):
    ...
    trip_mc_vis['tourmode_num'] = trip_mc_vis['tour_mode'].apply(lambda x: calib_trip_mode_to_vis_dict[x])
        ...
        for mode in output_calibration_trip_modes:
            for tour_mode in output_calibration_tour_modes[:-1]:
                ...
                for coef_purpose in output_calibration_purposes_to_coef_names_dict[purpose]:
                    for m in output_calibration_mode_to_coef_name_dict_tour[tour_mode]:
                        for n in output_calibration_mode_to_coef_name_dict_trip[mode]:
                            coef_trip_mode = n
                            coef_tour_mode = m
                            coef_name = 'coef_' + coef_tour_mode + '_ASC_' + coef_trip_mode + '_' + coef_purpose
                            track_coef_name_tries += [coef_name]
                            if purpose in ['joint_maint', 'joint_discr']:
                                coef_name = 'coef_joint_' + coef_tour_mode + '_ASC_' + coef_trip_mode + '_' + coef_purpose
                                track_coef_name_tries += [coef_name]
                            row = [coef_name, coef_purpose, coef_tour_mode, coef_trip_mode, asim_mc_count, calib_target_count]
                        model_calib_target_match_df.loc[len(model_calib_target_match_df)] = row
    model_calib_target_match_df.to_csv('coef_update.csv')
    pd.DataFrame({'coef_name': track_coef_name_tries}).to_csv('debug_coefficient_names.csv')
...
def calculate_new_coefficient(row, damping_factor, max_ASC_adjust, adjust_when_zero_counts):
    ...
    if abs(row.value) > 900 or (pd.isna(row.scaled_target_percent) and pd.isna(row.scaled_model_percent)):
        return row
...
def perform_trip_mode_choice_model_calibration(asim_output_dir,
    ...
    trip_mc_calib_target_tables = read_trip_mode_choice_calibration_file(
        trip_mode_choice_calib_targets_file)
    #print(trip_mc_calib_target_tables)
...
def run_activitysim(data_dir,
    ...
    if trip_mc_coef_file is not None:
        shutil.copyfile(trip_mc_coef_file, os.path.join(run_config_dir, 'trip_mode_choice_coefficients.csv'))
    ...
    activitysim_run_command = 'python simulation.py -c ' + run_config_dir \
        + ' -d ' + data_dir + ' -o ' + run_dir
    if run_config_mp_dir is not None:
        activitysim_run_command = 'python simulation.py -c ' + run_config_mp_dir + ' -c ' + run_config_dir\
