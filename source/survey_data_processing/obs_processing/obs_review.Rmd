---
title: "Met Council OBS data review"
author: "Leah Flake (2021) Andrew Rohne (2023)"
...
obs_dir = 'E:\\Met_Council\\survey_data\\Phase2\\obs_processing'
...
Unlinked:
`r format(round(obs_data[, sum(unlinked_weight_scaled, na.rm = TRUE)], 0), big.mark = ",")`

Linked:
`r format(round(obs_data[, sum(linked_weight_scaled, na.rm = TRUE)], 0), big.mark = ",")`


By service provider:

```{r echo = FALSE}
kable(by_provider, "simple", format.args = list(decimal.mark = '.', big.mark = ",", digits = 0))
```
...
# Boardings by route
boardings = obs_data[, .(boardings =  sum(unlinked_weight_scaled, na.rm = TRUE)), .(provider, route =  ROUTE_LABEL)]

boardings = dcast(boardings, route ~ provider, value.var = 'boardings')
boardings[is.na(boardings)] = 0

kable(boardings, 'simple', format.args = list(decimal.mark = '.', big.mark = ",", digits = 0))
```
...
cat('\n \n By transit provider \n')
cat("\n")  #, position = 'fill'
ggplot(purp, aes(x = TOUR_PURPOSE, y = weighted_pct))+ geom_bar(stat = 'identity') + theme_bw() + facet_grid('provider', scales = 'free') + coord_flip() + ylab('Proportion') + xlab('Tour Purpose')
cat("\n")
cat("\n")
cat(' \n \nTotal \n')
cat("\n")
