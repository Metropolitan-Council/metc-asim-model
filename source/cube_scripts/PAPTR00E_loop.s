...
RUN PGM=PUBLIC TRANSPORT PRNFILE="%SCENARIO_DIR%\transit\XIT_DRE_LD_PRN_%PREV_ITER%_@TPER@.prn" MSG='@TPER@ Drive Egress Transit Assignment'
FILEI ROUTEI[1] = "%SCENARIO_DIR%\transit\XIT_DRE_RTE_%PREV_ITER%_@TPER@.RTE"
FILEO LINKO[1] = "%SCENARIO_DIR%\transit\XIT_DRE_LD_%PREV_ITER%_@TPER@.DBF",
ONOFFS=Y, NTLEGS=Y, SKIP0=Y
FILEI MATI[1] = "%SCENARIO_DIR%\transit\XIT_TRIP_%PREV_ITER%_@TPER@_CLEAN.trp"
FILEO REPORTO = "%SCENARIO_DIR%\transit\XIT_DRE_LD_RPT_%PREV_ITER%_@TPER@.RPT"
FILEI NETI = "%SCENARIO_DIR%\transit\XIT_DRE_NET_%PREV_ITER%_@TPER@.net"
FILEO NETO = "%SCENARIO_DIR%\transit\XIT_LDNET_DRE_%PREV_ITER%_@TPER@.net"
