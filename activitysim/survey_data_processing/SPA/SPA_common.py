...
class Person:
    ...
    def print_header(fp):
         # DH [02/04/20] - replaced AGE_CAT with AGE to match SEMCOG HTS data
        _header=["HH_ID", "PER_ID", "PERSONTYPE", "AGE", "EMPLY", "HOURS_CAT", "EMP_CAT", "STUDE", "SCHOL", "STU_CAT", "PERSONTYPE0", "EMP_CAT0", "STU_CAT0", "ERROR" ]
        fp.write(','.join(['%s' %field for field in _header])+'\n')
...
class Tour:
    ...
    def _calc_tour_mode(self):
        ...
        if NewTripMode['SCHOOLBUS'] in _modes_used:
            ...
            _mode = NewTourMode['PNR-MIXED']
        elif NewTripMode['PNR-PREMIUM'] in _modes_used:
            _mode = NewTourMode['PNR-PREMIUM']
        elif NewTripMode['PNR-LOCAL'] in _modes_used:
            _mode = NewTourMode['PNR-LOCAL']
        elif NewTripMode['KNR-MIXED'] in _modes_used:
            _mode = NewTourMode['KNR-MIXED']
        elif NewTripMode['KNR-PREMIUM'] in _modes_used:
            _mode = NewTourMode['KNR-PREMIUM']
        elif NewTripMode['KNR-LOCAL'] in _modes_used:
            _mode = NewTourMode['KNR-LOCAL']
        elif NewTripMode['WALK-MIXED'] in _modes_used:
            _mode = NewTourMode['WALK-MIXED']
        elif NewTripMode['WALK-PREMIUM'] in _modes_used:
        ...
        # _transit_modes  = {NewTripMode['TRANSIT']}
        ...
        # if len(_transit_modes&_modes_used)>0:
        #     #If any trip on tour is PNR, tour mode is PNR
        #     _mode = NewTourMode['TRANSIT']
    ...
    def _get_is_driver(self):
        _is_driver = int(0)
        for _trip in self.trips:
            if 'ISDRIVER' in _trip.fields:      #make sure that the key exists before looking up its value
...
class Trip:
    ...
    def populate_attributes(self, df):
        ...
        if _is_transit:
            ...
            _output_mode = NewTripMode['TNC-SINGLE']
        ...
        for row_index in range(1,_last_row+1):
            ...
            _hh_mem_on_trip = df['HHMEM'].iloc[row_index]   
